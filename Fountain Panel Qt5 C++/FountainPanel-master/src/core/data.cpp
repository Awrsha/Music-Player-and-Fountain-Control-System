  ...
  if (q.size() == 0)
    {
      query.exec("insert into listlist"
                 "(name, id, count) values('The default list', 0, 0);");
      Data::changeListCount(1);
      q.push_back(QString("The default list"));
      qDebug() << "Create list:The default list" << endl;
    }
  return q;
}
  ...
  while (!musics.empty())
    {
      ...
      QString str
          = QString("insert into "
                    "musiclist(listname, id, musicDir, musicName, musicArtist, fountainDir, fountainName, fountainArtist) "
            ...
            musicFountainInfo.fountainInfo.getArtist());
